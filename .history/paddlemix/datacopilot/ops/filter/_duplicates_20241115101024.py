...
def remove_duplicates(
    ...
    for item in dataset:
        # 假设 'item' 是一个字典，包含了图像路径
        image_path = item.get('image_path', None)  # 获取图像路径
        if image_path:  # 确保图像路径存在
            hash_value = compute_hash(item, hash_method)
            if hash_value:
                hash_values[image_path] = hash_value  # 使用 image_path 作为唯一键

    # 遍历数据集，保留哈希值唯一的图像
    for item in dataset:
        image_path = item.get('image_path', None)
        if image_path and hash_values.get(image_path) not in hash_set:
            hash_set.add(hash_values[image_path])
            filtered_items.append(item)
