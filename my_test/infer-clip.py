...
class BatchSimilarity:
    ...
    def compute_similarity(self, image_path, texts):
        ...
        """
        # 处理图片
        image = self.processor.image_processor([image_path], return_tensors="pd")["image"]

        # 分批处理文本
        similarities = []
        for i in range(0, len(texts), self.batch_size):
            batch_texts = texts[i:i + self.batch_size]
            # text_features = self.processor.text_processor(batch_texts, return_tensors="pd")
            text_features = self.processor.text_processor(batch_texts, return_tensors="pd")[i]
